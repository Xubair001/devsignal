...
export function DangerZone() {
  ...
  return (
    <Card className="border-bad/25">
      <h2 className="text-lead font-semibold text-bad">Delete account</h2>
      <p className="mt-1.5 max-w-[70ch] text-meta leading-relaxed text-ink-2">
      ...
      <p className="mt-2 max-w-[70ch] text-meta leading-relaxed text-ink-3">
      ...
      {!open ? (
        ...
        <div className="mt-4 flex flex-col gap-3 rounded-[10px] border border-bad/25 bg-bad-wash p-3.5">
          <label htmlFor="confirm" className="text-meta font-medium">
            Type <code className="rounded bg-surface px-1.5 py-0.5 font-mono text-meta">{CONFIRM}</code> to confirm
          </label>
          <Input
            id="confirm"
            value={typed}
          ...
          {erase.isError && (
            <p role="alert" className="text-meta font-medium text-bad">
